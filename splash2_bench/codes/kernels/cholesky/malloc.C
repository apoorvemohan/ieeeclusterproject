...
//  mem_pool = (struct MemPool *)G_MALLOC((mallocP+1)*sizeof(struct MemPool), 0);

    posix_memalign(&mem_pool, sizeof(struct MemPool *), (mallocP+1)*sizeof(struct MemPool));
...
void InitOneFreeList(long p)
{
  ...
  if (p > 0) {
//    mem_pool[p].freeBlock = (long **) G_MALLOC((MAXFAST+1)*sizeof(long *), p);

 posix_memalign(&mem_pool[p].freeBlock , sizeof(long **), (MAXFAST+1)*sizeof(long *));

    MigrateMem(mem_pool[p].freeBlock, (MAXFAST+1)*sizeof(long *), p);
  }
  else {
//    mem_pool[p].freeBlock = (long **)G_MALLOC((MAXFAST+1)*sizeof(long *), 0);
...
posix_memalign(&freespace, sizeof(long *), block_size+2*sizeof(long));
