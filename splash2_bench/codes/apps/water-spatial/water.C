...
//            start_end[i] = (first_last_array *)G_MALLOC(sizeof(first_last_array));
  ...
  //      my_boxes = (box_list **) G_MALLOC(NumProcs * sizeof(box_list *));
	...
	posix_memalign(&my_boxes, sizeof(box_list **), NumProcs * sizeof(box_list *));
...
//        BOX = (box_type ***) G_MALLOC(BOX_PER_SIDE * sizeof(box_type **));
	...
	posix_memalign(&BOX, sizeof(box_type ***), BOX_PER_SIDE * sizeof(box_type **));

        for (i=0; i < BOX_PER_SIDE; i++) {
//            BOX[i] = (box_type **) G_MALLOC( BOX_PER_SIDE * sizeof(box_type *));
		posix_memalign(&BOX[i], sizeof(box_type **), BOX_PER_SIDE * sizeof(box_type *));

            for (j=0; j < BOX_PER_SIDE; j++) {
//                BOX[i][j] = (box_type *) G_MALLOC(BOX_PER_SIDE * sizeof(box_type));
		posix_memalign(&BOX[i][j], sizeof(box_type *), BOX_PER_SIDE * sizeof(box_type));
