...
//      Local[i].ctab = (cellptr) valloc((maxcell / NPROC) * sizeof(cell));;
      ...
      posix_memalign(&Local[i].ctab, sizeof(cellptr), (maxcell / NPROC) * sizeof(cell));
//      Local[i].ltab = (leafptr) valloc((maxleaf / NPROC) * sizeof(leaf));;

      posix_memalign(&Local[i].ltab, sizeof(leafptr), (maxleaf / NPROC) * sizeof(leaf));
...
//   Local[0].mybodytab = (bodyptr*) valloc(NPROC*maxmybody*sizeof(bodyptr));;
	...
	posix_memalign(&Local[0].mybodytab, sizeof(bodyptr*), NPROC*maxmybody*sizeof(bodyptr));
...
//   Local[0].mycelltab = (cellptr*) valloc(NPROC*maxmycell*sizeof(cellptr));;
	...
	posix_memalign(&Local[0].mycelltab, sizeof(cellptr*), NPROC*maxmycell*sizeof(cellptr));


//   Local[0].myleaftab = (leafptr*) valloc(NPROC*maxmyleaf*sizeof(leafptr));;

	posix_memalign(&Local[0].myleaftab, sizeof(leafptr*), NPROC*maxmyleaf*sizeof(leafptr));

//   CellLock = (struct CellLockType *) valloc(sizeof(struct CellLockType));;

	posix_memalign(&CellLock, sizeof(struct CellLockType *), sizeof(struct CellLockType));
...
//   bodytab = (bodyptr) valloc(nbody * sizeof(body));;
	...
	posix_memalign(&bodytab, sizeof(bodyptr), nbody * sizeof(body));
