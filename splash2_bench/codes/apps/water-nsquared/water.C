...
//        VAR = (molecule_type *) G_MALLOC(mol_size);
	...
	posix_memalign(&VAR, sizeof(molecule_type *), mol_size);

//        gl = (struct GlobalMemory *) G_MALLOC(gmem_size);

	posix_memalign(&gl, sizeof(struct GlobalMemory *), gmem_size);
...
//	posix_memalign(&PFORCES, sizeof(double ****), NumProcs * sizeof (double ***));

        { long i,j,k;

          for (i = 0; i < NumProcs; i++) {
              PFORCES[i] = (double ***) G_MALLOC(NMOL * sizeof (double **));
//		posix_memalign(&PFORCES[i], sizeof(double ***), NMOL * sizeof (double **));

              for (j = 0; j < NMOL; j++) {
                  PFORCES[i][j] = (double **) G_MALLOC(NDIR * sizeof (double *));
//			posix_memalign(&PFORCES[i][j], sizeof(double **), NDIR * sizeof (double *));
                  for (k = 0; k < NDIR; k++) {
                      PFORCES[i][j][k] = (double *) G_MALLOC(NATOM * sizeof (double));

//			posix_memalign(&PFORCES[i][j][k], sizeof(double *), NATOM * sizeof (double));
                  }
              }
          }
      }
