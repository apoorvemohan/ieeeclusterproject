...
//   particle_array = (particle *) G_MALLOC(Total_Particles * sizeof(particle));
   ...
   printf("Sysconf CALL: %ld\n", sysconf(_SC_PAGESIZE));

// #define MY_MALLOC(particle, align, size) posix_memalign(sysconf(_SC_PAGESIZE),size)


   int rc = posix_memalign(&particle_array, sysconf(_SC_PAGESIZE), Total_Particles * sizeof(particle));

   printf("Particle array: %0x\n", particle_array);
   printf("rc: %d\n", rc);

//   Particle_List = (particle **) G_MALLOC(Total_Particles * sizeof(particle *));

   posix_memalign(&Particle_List, sizeof(particle**), Total_Particles * sizeof(particle *));
...
CreateParticleList (long my_id, long length)
{
   ...
   //Local[my_id].Particles = (particle **) G_MALLOC(length * sizeof(particle *));

   posix_memalign(&Local[my_id].Particles, sizeof(particle**), length * sizeof(particle *));
